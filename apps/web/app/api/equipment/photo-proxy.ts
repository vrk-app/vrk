...
export async function proxyEquipmentPhotoJson(path: string, method: "POST" | "DELETE", request?: Request) {
...
export async function proxyEquipmentPhotoStream(path: string) {
  const token = await sessionToken();
  if (!token) {
    return unauthorizedResponse();
  }
